...
@Injectable()
export class GalleryItemContentHandler implements ContentHandler {
  ...
  getBindings(settings: Array<AttributeValue>, type: string, metadata?: Map<string, any>): Observable<Array<ContentBinding>> {
    ...
    if (type === 'pane') {
      return this.toObject(settings).pipe(
        switchMap(i => iif(
          () => i.bindingOption && i.bindingOption !== null && i.bindingOption !== '',
          of([ new ContentBinding({ id: i.bindingOption, type }) ]),
          of([])
        ))
      );
  ...
  toObject(settings: Array<AttributeValue>): Observable<GalleryItem> {
    return of(new GalleryItem(this.attributeSerializer.deserializeAsObject(settings)));
  }

  buildSettings(instance: GalleryItem ): Array<AttributeValue> {
